...
//
#include "../include/VFDNeutronHPChannelList.hh"
...
#include "G4IonTable.hh"
  ...
  G4ThreadLocal G4int VFDNeutronHPChannelList::trycounter = 0;

  VFDNeutronHPChannelList::VFDNeutronHPChannelList(G4int n)
  {
    nChannels = n;
    theChannels  = new VFDNeutronHPChannel * [n];
    allChannelsCreated = false;
    theInitCount = 0;
  }

  VFDNeutronHPChannelList::VFDNeutronHPChannelList()
  {
    nChannels = 0;
    theChannels = 0;
    allChannelsCreated = false;
  ...
  G4HadFinalState * VFDNeutronHPChannelList::ApplyYourself(const G4Element * , const G4HadProjectile & aTrack)
  {
  ...
  void VFDNeutronHPChannelList::Init(G4Element * anElement, const G4String & dirName)
  {
    theDir = dirName;
...
//    G4cout << theElement << G4endl;
  ...
  void VFDNeutronHPChannelList::Register(VFDNeutronHPFinalState * theFS,
                                        const G4String & aName)
  {
    if(!allChannelsCreated)
    {
      if(nChannels!=0)
      {
	VFDNeutronHPChannel ** theBuffer = new VFDNeutronHPChannel * [nChannels+1];
	G4int i;
	for(i=0; i<nChannels; i++)
	{
	...
	theChannels = theBuffer;
      ...
      {
	theChannels = new VFDNeutronHPChannel * [nChannels+1];
      }
      G4String name;
      name = aName+"/";
      theChannels[nChannels] = new VFDNeutronHPChannel;
