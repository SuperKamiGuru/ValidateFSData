...
G4bool VFDNeutronHPPhotonDist::InitMean(std::istream & aDataFile)
{
  ...
  if(aDataFile >> repFlag)
  {
    ...
    else if(repFlag == 2)
    {
	 ...
	 {
           throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPPhotonDist: Unknown conversion flag");
	 }
      }
       // Note, that this is equivalent to using the 'Gamma' classes.
      // throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPPhotonDist: Transition probability array not sampled for the moment.");
    }
    else
    {
      G4cout << "Data representation in VFDNeutronHPPhotonDist: "<<repFlag<<G4endl;
      throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPPhotonDist: This data representation is not implemented.");
    }
  }
  else
  {
...
void VFDNeutronHPPhotonDist::InitAngular(std::istream & aDataFile)
{
  ...
  if (isoFlag != 1)
  {
if ( repFlag == 2 ) G4cout << "VFDNeutronHPPhotonDist: repFlag == 2 && isoFlag != 1 is unexpected! If you use G4ND3.x, then please report to Geant4 Hyper News. Thanks." << G4endl;
    aDataFile >> tabulationType >> nDiscrete2 >> nIso;
//080731
      if ( theGammas != NULL && nDiscrete2 != nDiscrete ) G4cout << "080731c VFDNeutronHPPhotonDist nDiscrete2 != nDiscrete, It looks like something wrong in your NDL files. Please update the latest. If you still have this messages after the update, then please report to Geant4 Hyper News." << G4endl;
...
void VFDNeutronHPPhotonDist::InitEnergies(std::istream & aDataFile)
{
...
void VFDNeutronHPPhotonDist::InitPartials(std::istream & aDataFile)
{

  //G4cout << "VFDNeutronHPPhotonDist::InitPartials " << G4endl;
  ...
  //G4cout << "VFDNeutronHPPhotonDist::InitPartials Test " << G4endl;
  //G4cout << "VFDNeutronHPPhotonDist::InitPartials nDiscrete " << nDiscrete << G4endl;
