...
#include "../include/VFDNeutronHPChannel.hh"
#include "globals.hh"
...
#include "G4NeutronHPReactionWhiteBoard.hh"
  ...
  G4double VFDNeutronHPChannel::GetXsec(G4double energy)
  {
    return std::max(0., theChannelData->GetXsec(energy));
  }

  G4double VFDNeutronHPChannel::GetWeightedXsec(G4double energy, G4int isoNumber)
  {
    return theIsotopeWiseData[isoNumber].GetXsec(energy);
  }

  G4double VFDNeutronHPChannel::GetFSCrossSection(G4double energy, G4int isoNumber)
  {
    return theFinalStates[isoNumber]->GetXsec(energy);
  }

  void VFDNeutronHPChannel::
  ...
  void VFDNeutronHPChannel::Init(G4Element * anElement, const G4String dirName)
  {
    theDir = dirName;
    theElement = anElement;
  }

  G4bool VFDNeutronHPChannel::Register(VFDNeutronHPFinalState *theFS)
  {
	registerCount++;
...
*/
    ...
    theFinalStates = new VFDNeutronHPFinalState * [niso];
  ...
  //void VFDNeutronHPChannel::UpdateData(G4int A, G4int Z, G4int index, G4double abundance)
  void VFDNeutronHPChannel::UpdateData(G4int A, G4int Z, G4int M, G4int index, G4double abundance)
  {
  ...
  void VFDNeutronHPChannel::Harmonise(G4NeutronHPVector *& theStore, G4NeutronHPVector * theNew)
  {
...
#include "G4NeutronHPThermalBoost.hh"
  ...
  G4HadFinalState * VFDNeutronHPChannel::
  ApplyYourself(const G4HadProjectile & theTrack, G4int anIsotope)
  {
//    G4cout << "VFDNeutronHPChannel::ApplyYourself+"<<niso<<G4endl;
    if ( anIsotope != -1 && anIsotope != -2 )
    {
       //Inelastic Case
       //G4cout << "VFDNeutronHPChannel Inelastic Case"
    ...
    if(sum == 0)
    {
//      G4cout << "VFDNeutronHPChannel::ApplyYourself theFinalState->Initialize+"<<G4endl;
//      G4cout << "VFDNeutronHPChannel::ApplyYourself theFinalState->Initialize-"<<G4endl;
...
//      G4cout << "TESTHP 23 NISO="<<niso<<G4endl;
      ...
      G4double running=0;
//      G4cout << "VFDNeutronHPChannel::ApplyYourself Done the sum"<<niso<<G4endl;
...
//	      G4cout << "TESTHP 24 it="<<it<<G4endl;
    ...
    //G4cout << "TK VFDNeutronHPChannel Elastic, Capture and Fission Cases "
    //<< " Z= " << this->GetZ(it) << " A = " << this->GetN(it) << G4endl;
