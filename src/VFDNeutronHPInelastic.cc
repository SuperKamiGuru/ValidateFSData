...
#include "G4NeutronHPManager.hh"
  ...
  VFDNeutronHPInelastic::VFDNeutronHPInelastic()
    :G4HadronicInteraction("NeutronHPInelastic")
  {
    ...
    if ( istatus < 0 )
    {
      G4cout << "Warning! system(\"echo $G4NEUTRONHPDATA\") returns error value at VFDNeutronHPInelastic" << G4endl;
    }

//    G4cout << " entering VFDNeutronHPInelastic constructor"<<G4endl;
...
/*
    theInelastic = new VFDNeutronHPChannelList[numEle];
	...
	theInelastic[i].RestartRegistration();
      ...
      while( !theInelastic[i].HasDataInAnyFinalState() && itry < 6 );
                                                              // 6 is corresponding to the value(5) of VFDNeutronHPChannel. TK
...
*/
    ...
    for (G4int i=0; i<numEle; i++)
    {
      theInelastic.push_back( new VFDNeutronHPChannelList );
      (*theInelastic[i]).Init((*(G4Element::GetElementTable()))[i], dirName);
      G4int itry = 0;
      do
      {
  ...
  {
//    delete [] theInelastic;
     for ( std::vector<VFDNeutronHPChannelList*>::iterator
  ...
  G4HadFinalState * VFDNeutronHPInelastic::ApplyYourself(const G4HadProjectile& aTrack, G4Nucleus& aNucleus )
  {
...
const std::pair<G4double, G4double> VFDNeutronHPInelastic::GetFatalEnergyCheckLevels() const
{
...
void VFDNeutronHPInelastic::addChannelForNewElement()
{
   for ( G4int i = numEle ; i < (G4int)G4Element::GetNumberOfElements() ; i++ )
   {
      G4cout << "VFDNeutronHPInelastic Prepairing Data for the new element of " << (*(G4Element::GetElementTable()))[i]->GetName() << G4endl;
...
G4int VFDNeutronHPInelastic::GetVerboseLevel() const
{
   return G4NeutronHPManager::GetInstance()->GetVerboseLevel();
}
void VFDNeutronHPInelastic::SetVerboseLevel( G4int newValue )
{
