...
G4ReactionProduct * VFDNeutronHPContEnergyAngular::Sample(G4double anEnergy, G4double massCode, G4double /*mass*/)
{
...
G4double VFDNeutronHPContEnergyAngular::
MeanEnergyOfThisInteraction()
{
   G4double result(0);
   if(currentMeanEnergy<-1)
   {
     throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPContEnergyAngular: Logical error in Product class");
   }
   else
   {
...
void VFDNeutronHPContEnergyAngular::ClearHistories()
{
   if ( theAngular!= NULL )
   {
