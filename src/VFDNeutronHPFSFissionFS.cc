...
#include "G4NeutronHPDataUsed.hh"
    ...
    while (theData >> infoType)
    {
      ...
      switch(infoType)
      {
        ...
        case 5:
          ...
          break;
        default:
          G4cout << "VFDNeutronHPFSFissionFS::Init: unknown data type"<<dataType<<G4endl;
          throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPFSFissionFS::Init: unknown data type");
          break;
      }
    }
  ...
  G4HadFinalState * VFDNeutronHPFSFissionFS::ApplyYourself( const G4HadProjectile &aTrack )
  {
    theResult.Clear();
...
// return the result
    ...
    if(thePhotons!=0)
   {
     ...
     for(i=nPrompt; i<nPrompt+nDelayed; i++)
     {
       G4double time = -std::log(G4UniformRand())/theDecayConst[i-nPrompt];
       time += aTrack.GetGlobalTime();
       theResult.GetSecondary(i)->SetTime(time);
     }
    delete [] theDecayConst;
...
void VFDNeutronHPFSFissionFS::SampleNeutronMult(G4int&all, G4int&Prompt, G4int&delayed, G4double eKinetic, G4int off)
{
...
G4DynamicParticleVector * VFDNeutronHPFSFissionFS::GetPhotons()
{
// sample photons
