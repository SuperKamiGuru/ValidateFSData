...
#include "VFDNeutronHPContAngularPar.hh"
...
#include "G4IonTable.hh"
  ...
  void VFDNeutronHPContAngularPar::Init(std::istream & aDataFile)
  {
  ...
  G4ReactionProduct *
  VFDNeutronHPContAngularPar::Sample(G4double anEnergy, G4double massCode, G4double /*targetMass*/,
                                    G4int angularRep, G4int /*interpolE*/ )
  {
    ...
    else if(A==4)
    {
      ...
      if(Z!=2) throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPContAngularPar: Unknown ion case 1");
    }
    else
    {
        delete result;
            ...
            if ( theAngular[j].GetLabel() == 0.0 ) {
               ...
               if ( j != nEnergies-1 ) {
                  ...
                  if ( theAngular[j].GetValue(0) != 0.0 ) {
                     throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPContAngularPar: Unexpected non zero value of theAngular[nEnergies-1].GetValue(0)");
                  }
               }
            }
	       ...
	       if ( random < running[ j ] )
            ...
            for (G4int j=0;j<nAngularParameters;j++)
            {
               ...
               if ( it == 0 )
               {
                  ...
                  //G4cout << "110611 VFDNeutronHPContAngularPar::Sample it = 0; invetigation required " << G4endl;
                  itt = it+1;
               }
...
//      Bug Report #1366 from L. Russell
    ...
    {
      throw G4HadronicException(__FILE__, __LINE__, "VFDNeutronHPContAngularPar::Sample: Unknown angular representation");
    }
